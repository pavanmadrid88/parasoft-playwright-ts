...
export class HomeIndexPage extends BasePage{
    ...
    async isHomeIndexPageDisplayed() {
        //const atmServicesPresent = await this.homeIndexPage.locator("text=ATM Services").isVisible({ timeout: 10000 });
        const atmServicesPresent = await this.waitUtils.isElementInteractable(this.atmServicesText);
        //const onlineServicesPresent = await this.homeIndexPage.locator("text=Online Services").isVisible({ timeout: 10000 });
